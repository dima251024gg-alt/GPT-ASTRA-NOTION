"""Безопасный пакетный OCR-конвейер R1: карантин, AV, worker, review."""
...
class DemoAntivirus:
 ...
 def scan(self,raw,mime):
  ...
  return 'clean'
class ClamAntivirus:
 def __init__(self):self.binary=shutil.which('clamscan');self.info=AdapterInfo('clamav','cli','available' if self.binary else 'unavailable')
 def scan(self,raw,mime):
  ...
  with tempfile.TemporaryDirectory(prefix='aq-av-') as folder:
   path=Path(folder)/'sample';path.write_bytes(raw);result=subprocess.run([self.binary,'--no-summary',str(path)],capture_output=True,timeout=30,check=False,env={'PATH':os.environ.get('PATH','')})
...
class BuiltinOCR:
 def __init__(self):
  mode=os.getenv('OCR_PROVIDER','local');demo=config.PROVIDER_MODE=='mock' and not config.PRODUCTION;available=mode=='russian-api' or bool(shutil.which('tesseract'));self.mode=mode;self.info=AdapterInfo('builtin-'+mode,'1','demo' if demo else ('available' if available else 'unavailable'))
 def recognize(self,raw,mime):
  if self.info.state=='unavailable':raise AppError('OCR недоступен. Используйте ручной ввод.',503)
  if self.mode!='local':raise AppError('Для внешнего OCR ещё не настроен изолированный адаптер',503)
  from .worker_sandbox import isolated_recognize
  return isolated_recognize(raw,mime)
...
def record_call(db,adapter,operation,key,attempt,status,started,error_code=None,actor=None):return db.insert('integration_calls',{'adapter':adapter.info.name,'operation':operation,'idempotency_key':blind(key),'attempt':attempt,'status':status,'duration_ms':max(0,int((time.monotonic()-started)*1000)),'error_code':error_code},actor)
def duplicate_hints(service,fields,current_person_id):
 hints=[];number=clean_text(fields.get('number'));doc_type=clean_text(fields.get('type'))
 if number and doc_type:
  candidate={'number':number,'series':clean_text(fields.get('series')),'type':doc_type,'issuing_country':clean_text(fields.get('issuing_country')) or 'RUS'};match=service.db.find('identity_documents','document_key=?',(document_key(candidate),))
  if match:
   person=service.db.one('persons',match['person_id']);hints.append({'kind':'document_exact','candidate_person_id':person['id'],'same_person':person['id']==current_person_id,'selectable':True} if service.can('persons',person) else {'kind':'document_exact_restricted','selectable':False})
 if all(fields.get(k) for k in ('last_name_ru','first_name_ru','birth_date')):
  ...
  for person in service.db.all('persons','identity_key=?',(identity_key(probe),)):
...
def field_rows(result):
 ...
 for name,value in fields.items():
  ...
  conflict=False
  if name in ('last_name_latin','first_name_latin'):
   visual=fields.get(name+'_visual');conflict=bool(visual and normalize_name(visual)!=normalize_name(value))
  rows.append({'name':name,'value':value,'source':source,'raw_confidence':confidence.get(name),'confidence_origin':clean_text(result.get('confidence_origin') or 'provider',80),'calibrated_confidence':None,'critical':int(name in CRITICAL_FIELDS),'confirmed':0,'conflict':int(conflict),'possible_truncation':int(truncation and name in ('last_name_latin','first_name_latin'))})
 return rows,confidence
def warning_codes(result):
 codes=[]
 if not result.get('mrz_valid') and (result.get('fields') or {}).get('mrz_raw'):codes.append('mrz_check_failed')
 if (result.get('fields') or {}).get('possible_truncation'):codes.append('possible_truncation')
...
def _run_ocr(service,item_row,raw,mime,person_id,adapter,operation_key):
 ...
 try:
  result=adapter.recognize(raw,mime);rows,confidence=field_rows(result);status='manual_required' if result.get('provider')=='manual' or not rows else 'review_required';record_call(db,adapter,'recognize',run_key,1,'succeeded',started,actor=service.actor);run=db.insert('ocr_runs',{**common,'raw_confidence':confidence,'status':status,'warnings':warning_codes(result),'dedupe_hints':duplicate_hints(service,result.get('fields') or {},person_id)},service.actor)
  for row in rows:db.insert('ocr_fields',{'ocr_run_id':run['id'],**row},service.actor)
  return run
 except Exception as exc:
  record_call(db,adapter,'recognize',run_key,1,'failed',started,type(exc).__name__[:80],service.actor);return db.insert('ocr_runs',{**common,'raw_confidence':{},'status':'manual_required','warnings':['ocr_failed'],'dedupe_hints':[]},service.actor)
def process_batch(service,items,operation_key,adapter=None,antivirus=None):
 ...
 for ordinal,item in enumerate(items):
  ...
  try:
   ...
   from . import secure_upload
   quarantine=secure_upload.stage_file(service.db,raw,item_row['name'],mime,person_id,item_row['id'],service.actor)
   if antivirus.info.state=='unavailable':secure_upload.scan_event(service.db,quarantine,antivirus,'unavailable',0,'scanner_unavailable',service.actor);raise AppError('Антивирус недоступен: файл остаётся в карантине',503)
   if antivirus.info.state=='demo' and config.PRODUCTION:secure_upload.scan_event(service.db,quarantine,antivirus,'blocked',0,'demo_forbidden',service.actor);raise AppError('Демонстрационный антивирус запрещён в production',503)
   started=time.monotonic()
   try:
    scan=antivirus.scan(raw,mime);record_call(service.db,antivirus,'scan',f'{operation_key}:av:{ordinal}',1,'succeeded',started,actor=service.actor);quarantine=secure_upload.scan_event(service.db,quarantine,antivirus,scan,int((time.monotonic()-started)*1000),actor=service.actor)
   except Exception as exc:
    record_call(service.db,antivirus,'scan',f'{operation_key}:av:{ordinal}',1,'failed',started,type(exc).__name__[:80],service.actor);secure_upload.scan_event(service.db,quarantine,antivirus,'error',int((time.monotonic()-started)*1000),type(exc).__name__[:80],service.actor);raise
   if scan!='clean':secure_upload.destroy_staged(service.db,quarantine,'rejected');raise AppError('Файл отклонён антивирусной проверкой',422)
   file_row=secure_upload.promote_file(service.db,quarantine,'persons',person_id,service.actor);service.db.update('upload_items',item_row['id'],{'file_id':file_row['id'],'status':'scanned'});item_row=service.db.one('upload_items',item_row['id']);run=_run_ocr(service,item_row,raw,mime,person_id,adapter,operation_key);status=run['status'];service.db.update('upload_items',item_row['id'],{'ocr_run_id':run['id'],'status':status});statuses.append(status)
...
def confirm_run(service,run_id,confirmations):
 ...
 for row in critical:
  ...
  if row['conflict'] and answer.get('conflict_ack') is not True:missing.append(row['name']+':расхождение');continue
  if row['possible_truncation'] and answer.get('truncation_ack') is not True:missing.append(row['name']+':усечение')
 if missing:raise AppError('Подтвердите критические поля по отдельности: '+', '.join(missing),422,{'fields':missing})
