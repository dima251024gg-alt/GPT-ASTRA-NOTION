...
def parse_mrz(raw,today=None):
    ...
    for key,value,is_birth in [('birth_date',b[13:19],True),('expiry_date',b[21:27],False)]:
        ...
        except AppError: fields[key]=''; date_errors.append(key)
    valid=all(checks.values()) and not date_errors
    confidence={k:(0.99 if valid else 0.65) for k in fields}
    # MRZ checks authenticate syntax, not identity or OCR certainty of names.
...
def build_mrz(last,first,number,birth,expiry,gender='M',country='RUS'):
    a=('P<'+country+transliterate(last).replace(' ','<')+'<<'+transliterate(first).replace(' ','<')).ljust(44,'<')[:44]
    number=str(number).ljust(9,'<')[:9]; bdate=birth.replace('-','')[2:]; edate=expiry.replace('-','')[2:]
    ...
    try:
        ...
        for contour in sorted(contours,key=cv2.contourArea,reverse=True)[:8]:
            ...
            if len(approx)==4 and cv2.contourArea(approx)>0.3*gray.size:
                pts=approx.reshape(4,2).astype('float32'); sums=pts.sum(axis=1); dif=np.diff(pts,axis=1).ravel()
                box=np.array([pts[sums.argmin()],pts[dif.argmin()],pts[sums.argmax()],pts[dif.argmax()]],dtype='float32')
                ...
                if min(w,h)>100:
                    matrix=cv2.getPerspectiveTransform(box,np.array([[0,0],[w-1,0],[w-1,h-1],[0,h-1]],dtype='float32'))
                    img=cv2.warpPerspective(img,matrix,(w,h)); found=True
                break
        gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        gray=cv2.createCLAHE(clipLimit=2.0,tileGridSize=(8,8)).apply(gray)
        # Contrast correction cannot restore text destroyed by glare: never invent it.
        warnings=[] if found else ['Границы документа не определены. Проверьте, что все края и строки попали в кадр.']
...
def parse_text(text,quality=0.7):
    upper=text.upper(); fields={}; warnings=[]
    lines=[re.sub(r'[^A-Z0-9<]','',x.upper().replace('«','<<')) for x in text.splitlines()]
    ...
    if 'СВИДЕТЕЛЬСТВО' in upper and 'РОЖДЕНИ' in upper:
        fields['type']='birth_certificate'
        m=re.search(r'([IVXLCDMІХV]+[-– ]*[А-ЯЁ]{2})\s*(?:№\s*)?(\d{6})',upper)
        ...
        fields.update(type='passport_rf',registration_address=re.sub(r'\s+',' ',text).strip())
        warnings.append('Страница регистрации: привяжите к существующему паспорту, не создавайте новый документ.')
    else:
        fields['type']='international_passport' if 'PASSPORT' in upper else 'passport_rf'
        warnings.append('Тип документа не определён надёжно. Для иностранного документа выберите тип и введите поля вручную.')
    ...
    def recognizeDocument(self,file,mime):
        if mime=='application/pdf':
            ...
            with tempfile.TemporaryDirectory() as temp:
                path=Path(temp)/'scan.pdf';path.write_bytes(file)
                subprocess.run(['pdftoppm','-f','1','-singlefile','-r','180','-png',str(path),str(Path(temp)/'page')],check=True,timeout=30,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
                file=(Path(temp)/'page.png').read_bytes()
        ...
        try:
            ...
            output=pytesseract.image_to_data(image,lang='rus+eng',config='--psm 6',output_type=pytesseract.Output.DICT,timeout=40)
            lines={}; boxes=[]; confidences=[]
            for i,text in enumerate(output['text']):
                if not text.strip(): continue
                key=(output['block_num'][i],output['par_num'][i],output['line_num'][i]); lines.setdefault(key,[]).append(text)
                conf=max(0,min(1,float(output['conf'][i])/100));confidences.append(conf)
                boxes.append({'x':output['left'][i]/image.shape[1],'y':output['top'][i]/image.shape[0],'w':output['width'][i]/image.shape[1],'h':output['height'][i]/image.shape[0],'text':text,'confidence':conf})
            text='\n'.join(' '.join(words) for words in lines.values())
            # Separate bottom-band pass avoids Russian character confusions in MRZ.
            mrz_text=pytesseract.image_to_string(image[int(image.shape[0]*0.60):],lang='eng',config='--psm 6 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789<',timeout=20)
            text+='\n'+mrz_text
            result=parse_text(text,sum(confidences)/len(confidences) if confidences else 0.3);result.boxes=boxes;result.warnings+=warnings
            return result
        except Exception:
...
class RussianApiOCR:
    def recognizeDocument(self,file,mime):
        ...
        result=requests.post(url,headers={'Authorization':'Bearer '+os.environ['OCR_API_KEY']},files={'file':('document',file,mime)},timeout=40)
        result.raise_for_status();data=result.json()
        parsed=DocumentData(fields=data.get('fields',{}),confidence={k:max(0,min(1,float(v))) for k,v in data.get('confidence',{}).items()},warnings=data.get('warnings',[]),provider='russian-api')
