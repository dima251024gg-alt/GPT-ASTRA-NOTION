...
OCR_BATCH=re.compile(r"/api/ocr/batches/([^/]+)")
OCR_CONFIRM=re.compile(r"/api/ocr/runs/([^/]+)/confirm")
MAX_LINK_DAYS=90
...
def file_payload(blob,meta,fallback):
    return {"name":str((meta or {}).get("name") or fallback),"mime":(meta or {}).get("mime") or "application/octet-stream","size":len(blob),"sha256":(meta or {}).get("sha256") or "","content_base64":base64.b64encode(blob).decode("ascii")}
def registry_view():
...
def preview(service,deal_id,code,person_id,extra):
    guard(service,deal_id);template=docflow.template_row(service.db,code);context=docflow.build_context(service.db,deal_id,extra,person_id or None);missing=list(render.missing_fields(template,context))
    return {"template":code,"name":template["name"],"ready":not missing,"missing":missing,"text":"" if missing else render.render_template(template,context)}
def dispatch_features(service,method,path,q,data):
    ...
    if match:
        only(method,"GET")
        from .ocr_pipeline import batch_view
        return batch_view(service,match[1])
    match=OCR_CONFIRM.fullmatch(path)
    if match:
        only(method,"POST")
        from .ocr_pipeline import confirm_run
        return confirm_run(service,match[1],data.get("confirmations"))
    if path=="/api/templates/registry":
        only(method,"GET");return registry_view()
    if path=="/api/templates/sync":
        only(method,"POST");service.require("admin","senior");return docflow.sync_templates(db,service.actor)
    if path=="/api/jobs/daily":
        only(method,"POST");return workflow.daily_run(service,str(data.get("date") or "") or None,str(data.get("name") or "daily"))
    ...
    if match:
        only(method,"GET");deal_id=match[1];deal=guard(service,deal_id);on=param(q,"date") or None;issues=rules.check_deal(db,deal_id,on);stops={item["code"] for item in rules.blockers(issues)}
        return {"status":deal["status"],"date":rules.today(on),"issues":issues,"summary":rules.summary(issues),"next":workflow.statuses_after(deal["status"]),"gates":[{"status":name,"ready":not(set(codes)&stops),"blocking":sorted(set(codes)&stops)} for name,codes in rules.STATUS_GATES.items()]}
    ...
    if match:
        only(method,"GET");blob,meta=docflow.package_file(service,match[1]);return {"file":file_payload(blob,meta,match[1]+".pdf")}
