...
class Database:
    ...
    def atomic(self):
        ...
        try:
            # Small agency: serializable application writes simplify counters, OTP, audit and outbox.
        ...
        with self.atomic():
            for table,spec in SCHEMA.items():
                ...
                for item in spec.split():
                    name,typ=item.split(":"); sqltype="BIGINT" if typ=="int" else "TEXT"
                    col=f'"{name}" {sqltype}'
            ...
            for table,field in [("deals","manager_id"),("clients","manager_id"),("persons","identity_key"),("persons","client_id"),("identity_documents","person_id"),("consents","person_id"),("documents","deal_id"),("tasks","due_at"),("notifications","status")]:
                self.execute(f'CREATE INDEX IF NOT EXISTS "ix_{table}_{field}" ON "{table}" ("{field}")')
            if self.pg:
                self.execute("CREATE OR REPLACE FUNCTION deny_audit_mutation() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'Audit is immutable'; END $$")
                self.execute("DROP TRIGGER IF EXISTS immutable_audit ON audit_log")
                self.execute("CREATE TRIGGER immutable_audit BEFORE UPDATE OR DELETE OR TRUNCATE ON audit_log FOR EACH STATEMENT EXECUTE FUNCTION deny_audit_mutation()")
            else:
                for action in ["UPDATE","DELETE"]:
                    self.execute(f"CREATE TRIGGER IF NOT EXISTS audit_no_{action.lower()} BEFORE {action} ON audit_log BEGIN SELECT RAISE(ABORT,'Audit is immutable'); END")
    ...
    def audit(self,actor,action,entity,entity_id,details=None,ip="",agent=""):
        # Call within the same atomic transaction as the protected operation.
        row=self.execute("SELECT entry_hash FROM audit_log ORDER BY created_at DESC,id DESC LIMIT 1").fetchone()
        # Hash-chain order uses predecessor, not a wall-clock sort (UUIDs are random).
