...
class Database:
 ...
 def table_exists(self,table):
  if self.pg:return bool(self.execute("SELECT 1 FROM information_schema.tables WHERE table_schema=current_schema() AND table_name=?",(table,)).fetchone())
 ...
 def table_columns(self,table):
  ...
  if self.pg:return {row['column_name'] for row in self.execute("SELECT column_name FROM information_schema.columns WHERE table_schema=current_schema() AND table_name=?",(table,)).fetchall()}
  return {row['name'] for row in self.execute(f'PRAGMA table_info("{table}")').fetchall()}
 def column_sql(self,table,name,typ):
  result=f'"{name}" '+('BIGINT' if typ=='int' else 'TEXT')
  if not self.pg and name in FKS.get(table,{}):result+=f' REFERENCES "{FKS[table][name]}"(id)'
  return result
 @contextmanager
 def atomic(self):
  if self.depth:
 ...
 def migrate(self):
  with self.atomic():
   for table,spec in SCHEMA.items():
    ...
    for item in spec.split():name,typ=item.split(':');cols.append(self.column_sql(table,name,typ))
    self.execute(f'CREATE TABLE IF NOT EXISTS "{table}" ('+', '.join(cols)+')');existing=self.table_columns(table)
    for item in spec.split():
     name,typ=item.split(':')
     if name not in existing:self.execute(f'ALTER TABLE "{table}" ADD COLUMN '+self.column_sql(table,name,typ))
   ...
   self.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_deal_person ON deal_persons(deal_id,person_id) WHERE deleted_at IS NULL');self.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_client_person_relation ON client_person_relations(client_id,person_id,relation_type) WHERE deleted_at IS NULL');self.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_client_primary_person_relation ON client_person_relations(client_id) WHERE is_primary=1 AND status='active' AND deleted_at IS NULL");self.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_data_migration_entity ON data_migration_events(migration,entity_type,entity_id) WHERE deleted_at IS NULL');self.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_open_person_merge_source ON person_merges(source_person_id) WHERE status IN ('proposed','confirmed') AND deleted_at IS NULL");self.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_template_version ON template_versions(template_id,version)');self.execute('CREATE UNIQUE INDEX IF NOT EXISTS uq_job_run ON job_runs(name,run_date)')
   for table,field in [('deals','manager_id'),('clients','manager_id'),('clients','person_id'),('persons','identity_key'),('persons','merged_into_id'),('client_person_relations','client_id'),('client_person_relations','person_id'),('access_grants','subject_id'),('access_grants','resource_id'),('person_merges','source_person_id'),('person_merges','target_person_id'),('identity_documents','person_id'),('consents','person_id'),('documents','deal_id'),('tasks','due_at'),('notifications','status')]:self.execute(f'CREATE INDEX IF NOT EXISTS "ix_{table}_{field}" ON "{table}" ("{field}")')
   evidence_tables=('audit_log','ocr_runs','ocr_confirmations','integration_calls','file_scan_events','access_grant_events','data_migration_events','person_merge_events')
   if self.pg:
 ...
 def _migrate_person_client_links(self):
  if not self.table_exists('persons') or 'client_id' not in self.table_columns('persons'):return
  migration='persons_client_id_to_relations_v1';rows=self.execute("SELECT id,client_id,created_at,created_by,deleted_at FROM persons WHERE client_id IS NOT NULL AND client_id<>?",('',)).fetchall()
  for raw in rows:
   person=dict(raw);client_id=person['client_id']
   if not self.execute('SELECT 1 FROM clients WHERE id=?',(client_id,)).fetchone():raise AppError('Миграция остановлена: у person.client_id нет существующего клиента',500,{'person_id':person['id']})
   ...
   if linked:relation_id=dict(linked)['id'];outcome='already_linked'
   else:
    relation_id=uid();at=person.get('created_at') or now();self.execute('INSERT INTO client_person_relations (id,created_at,updated_at,created_by,deleted_at,client_id,person_id,relation_type,is_primary,status,valid_from,valid_to,source,verified_by,verified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(relation_id,at,at,person.get('created_by'),person.get('deleted_at'),client_id,person['id'],'unknown',0,'review_required',at,None,'legacy_person_client_id',None,None));outcome='backfilled_review_required'
  ...
  if self.pg:self.execute('ALTER TABLE persons DROP CONSTRAINT IF EXISTS fk_persons_client_id');self.execute('ALTER TABLE persons DROP COLUMN IF EXISTS client_id')
  else:self.execute('DROP INDEX IF EXISTS ix_persons_client_id');self.execute('ALTER TABLE persons DROP COLUMN client_id')
 ...
 def all(self,table,where='',args=(),include_deleted=False):
  ...
  return [self.decode(table,row) for row in self.execute(f'SELECT * FROM "{table}" WHERE '+condition+' ORDER BY created_at,id',args).fetchall()]
 ...
 def audit(self,actor,action,entity,entity_id,details=None,ip='',agent=''):
  rows=self.all('audit_log');referenced={row['previous_hash'] for row in rows};tails=[row for row in rows if row['entry_hash'] not in referenced];previous=tails[0]['entry_hash'] if tails else '0'*64;item={'user_id':actor,'action':action,'entity':entity,'entity_id':entity_id,'details':details or {},'ip':ip,'user_agent':agent[:500],'time':now(),'previous_hash':previous};item['entry_hash']=hmac.new(config.AUDIT_KEY,canonical(item).encode(),hashlib.sha256).hexdigest();return self.insert('audit_log',item,actor)
 def verify_audit(self):
  rows=self.all('audit_log');previous='0'*64;remaining={row['previous_hash']:row for row in rows}
  if len(remaining)!=len(rows):return False
  while previous in remaining:
   row=remaining.pop(previous);data={key:row[key] for key in COLUMNS['audit_log'] if key!='entry_hash'};actual=hmac.new(config.AUDIT_KEY,canonical(data).encode(),hashlib.sha256).hexdigest()
